...
//      resMgrModule = check_and_cast<ResourceGenericManager*>(getParentModule()->getSubmodule("nodeResourceMgr"));
	...
	sensorLastValue2.clear();
	double theBias;
	for (int i = 0; i < totalSensors; i++) {
		sensorLastValue.push_back(-1);
		sensorLastValue2.push_back(-1);
	...
	switch (msgKind) {
		...
		case SENSOR_READING_MESSAGE:{
			...
			if (getNewSample) {	//the last request for sample was more than minSamplingIntervals[sensorIndex] time ago
				...
				rcvPacket->setSensedValue2(sensorLastValue2[sensorIndex]);
		...
		case PHYSICAL_PROCESS_SAMPLING:
		{
			...
			int theValue2 = phyReply->getValue2();
			...
			if (theValue2 < sensorSensitivity[sensorIndex])
			    theValue2 = sensorSensitivity[sensorIndex];
			if (theValue2 > sensorSaturation[sensorIndex])
			    theValue2 = sensorSaturation[sensorIndex];

			theValue = sensorResolution[sensorIndex] * lrint(theValue / sensorResolution[sensorIndex]);
			theValue2 = sensorResolution[sensorIndex] * lrint(theValue2 / sensorResolution[sensorIndex]);
			sensorLastValue[sensorIndex] = theValue;
			sensorLastValue2[sensorIndex] = theValue2;
            ...
            readingMsg->setSensedValue2(theValue2);
			readingMsg->setSensorIndex(sensorIndex);

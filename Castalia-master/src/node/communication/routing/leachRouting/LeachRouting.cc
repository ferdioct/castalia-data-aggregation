	...
	switch (index) {
		...
		case START_SLOT:{
			...
			if (isCH) {
				...
				trace() << "Total Cluster Members :" << clusterLength;
...
void LeachRouting::sendAggregate()
{
	...
	{	
		...
		double aggrDataTemp = 0;
		int aggrDataHumid = 0;
		int totalData = bufferAggregate.size();
		double i;
		for (i = 0; i < bufferAggregate.size(); i++){
	        cPacket *cPkt = bufferAggregate.at(i).decapsulate();
	        ApplicationPacket *aPkt = dynamic_cast<ApplicationPacket*>(cPkt);
	        aggrDataTemp += aPkt->getData();
	        aggrDataHumid += aPkt->getData2();
		}
		trace() << "aaa" <<aggrDataHumid;
		trace() << "aaa" << totalData;
		aggrDataTemp/=totalData;
		aggrDataHumid/=totalData;
		newPacket->setData(aggrDataTemp);
		newPacket->setData2(aggrDataHumid);

...
void CustomizablePhysicalProcess::initialize()
{
	...
	ifstream myfile;
	myfile.open("/home/ferdioct/Desktop/kelembaban.txt");
	if(myfile){
	    string str;
	    string temp;
	    int value;
	    while(getline(myfile, str)){
	        //temp = str.substr(28,2);
	        //stringstream strstream(temp);
	        //value = 0;
	        //strstream >> value;
	        cout << str.substr(28,2) << '\n';
	        humidData.push_back(stoi(str.substr(28,2)));
	    }
	}
...
void CustomizablePhysicalProcess::handleMessage(cMessage * msg)
{
	...
	switch (inputType) {
		...
		case GENERATED_TEMPANDHUMID:{
		    int index = uniform(0,(humidData.size()-1));
		    humid = humidData.at(index);
		    double f1;
            double f2;
            double maxTemp = 33;
            double minTemp = 18;
            ...
            //int timeNow = tmLocal->tm_hour;
            int timeNow = 12;
            trace() << "waktu" << timeNow;
            if (timeNow < timeMin){
                f1 = cos(3.14*(timeMin-timeNow)/24 + timeMin-timeMax) + 1;
            }
            else if(timeMin < timeNow && timeNow < timeMax){
                 f1 = (cos(3.14*(timeNow-timeMin)/timeMax-timeMin)+1)/2;
            }
            else if (timeMin < timeNow){
                 f1 = (cos(3.14*(24+timeMin-timeNow)/24+timeMin-timeMax)+1)/2;
            }
            f2 = 1 - f1;
		    returnValue = f1 * minTemp + f2 * maxTemp;
		    break;
		}
	}
...
void CustomizablePhysicalProcess::readIniFileParameters(void)
{
	...
	switch (inputType) {
		...
		case 3: {
		    inputType = GENERATED_TEMPANDHUMID;
		    break;
		}
		default: {
